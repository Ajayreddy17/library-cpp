...
namespace mitsuha{
// Stores Lines in y = ax + b form
// querys val of (max / min) y for lines in container at x
namespace CHT {
template <typename T>
struct Line {
    mutable T k, m, p;
...
/*
CHT for plane without intercept
I think it works with long long / double. O(log N) per query
add(a, b, i = -1): Add ax + by (index = i)
get_max(x,y):(ax + by, i)
get_min(x,y):(ax + by, i)
*/
template <typename T>
struct CHT_xy {
    static_assert(is_same_v<T, long long> || std::is_floating_point_v<T>);
    using ld = long double;
    CHT_min<ld> cht_min;
    CHT_max<ld> cht_max;
    T amax = -numeric_limits<T>::max() / 2, amin = numeric_limits<T>::max() / 2;
    T bmax = -numeric_limits<T>::max() / 2, bmin = numeric_limits<T>::max() / 2;
    ...
    pair<T, int> get_max(T x, T y) {
        if (cht_min.empty()) return {-numeric_limits<T>::max() / 2, -1};
