...
/*
long long / double I think it works. per query O(log N)
・add(a, b)：ax + by addition of
...
struct CHT_xy {
    ...
    CHT_min<ld> cht_min;
    CHT_max<ld> cht_max;
    T amax = numeric_limits<T>::min() / 2, amin = numeric_limits<T>::max() / 2;
    T bmax = numeric_limits<T>::min() / 2, bmin = numeric_limits<T>::max() / 2;
    bool empty = true;
    ...
    T get_max(T x, T y) {
        if (cht_min.empty()) return numeric_limits<T>::min() / 2;
