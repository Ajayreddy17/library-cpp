#ifndef AJAY_RBST_ACTEDMONOID
#define AJAY_RBST_ACTEDMONOID
...
struct RBST_ActedMonoid {
    ...
    np apply(np root, unsigned int l, unsigned int r, const A a) {
        assert(0 <= l && l <= r && r <= root->size);
    ...
    X get(np root, unsigned int k) { return get_rec(root, k, false, Monoid_A::unit()); }
...
private:
    ...
    X get_rec(np root, unsigned int k, bool rev, A lazy) {
        np left = (rev ? root->r : root->l);
        np right = (rev ? root->l : root->r);
        unsigned int sl = (left ? left->size : 0);
    ...
    np apply_rec(np root, unsigned int l, unsigned int r, const A &a) {
        prop(root);
        root = copy_node(root);
        if (l == 0 && r == root->size) {
