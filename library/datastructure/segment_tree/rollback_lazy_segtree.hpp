#ifndef AJAY_ROLLBACK_LAZY_SEGTREE
#define AJAY_ROLLBACK_LAZY_SEGTREE
...
struct Rollback_Lazy_SegTree {
    ...
    Rollback_Lazy_SegTree(const vector<X>& v) { build(v); }
    ...
    void build(int m, F f) {
        ...
        while ((1 << log) < n) ++log;
        size = 1 << log;
        dat = Rollback_Array<X>(vector<X>(size << 1, MX::unit()));
        laz = Rollback_Array<A>(vector<A>(size, MA::unit()));
        for(int i = 0; i < n; i++) dat.set(size + i, f(i));
    ...
    void rollback(pair<int, int> t) { dat.rollback(t.first), laz.rollback(t.second); }
...
private:
    void apply_at(int k, A a) {
        long long sz = 1 << (log - (k == 0 ? -1 : 31 - __builtin_clz(k)));
        dat.set(k, AM::act(dat.get(k), a, sz));
        if (k < size) laz.set(k, MA::op(laz.get(k), a));
    }
};
} // namespace mitsuha
#endif // AJAY_ROLLBACK_LAZY_SEGTREE