#ifndef AJAY_SORTABLE_ARRAY
#define AJAY_SORTABLE_ARRAY

#include "library/datastructure/fastset.hpp"

namespace mitsuha{
// Duplicate keys are allowed, keys are int type
struct Sortable_Array {
    ...
    FastSet ss;      // fastset representing the entire left end of the interval
    vector<np> root; // Place the dynamic segtree node on the left end of the interval
    vector<bool> rev;
...
private:
    void init(vector<int>& key) {
        ...
        for (int i = 0; i < N; ++i) {
            root.emplace_back(new_node(0));
            assert(key[i] < KEY_MAX);
            set_rec(root[i], 0, KEY_MAX, key[i]);
        }
    }

    // make sure x is on the left edge
    void split_at(int x) {
        if (x == N || ss[x]) return;
