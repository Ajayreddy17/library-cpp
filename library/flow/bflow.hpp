...
private:
    ...
    bool dual(const Flow delta) {
        ...
        excess_vs.erase(
                remove_if(excess_vs.begin(), excess_vs.end(), [&](const V_id v) { return b[v] < delta; }),
                end(excess_vs));
        deficit_vs.erase(
                remove_if(deficit_vs.begin(), deficit_vs.end(), [&](const V_id v) { return b[v] > -delta; }),
                end(deficit_vs));
...
public:
    std::pair<bool, __int128> solve() {
        ...
        __int128 value = 0;
        for (const auto &es: g)
            for (const auto &e: es) { value += __int128(e.flow) * e.cost; }
        value /= 2;
    ...
    std::vector<Cost> get_potential() {
        ...
        for (int i = 0; i < (int)n; i++)
            for (const auto &es: g)
                for (const auto &e: es)
                    if (e.residual_cap() > 0)
                        potential[e.to] = std::min(potential[e.to], potential[e.frm] + e.cost);
        return potential;
    }
