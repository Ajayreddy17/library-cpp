...
inline void load() {
    ...
    pir = pir - pil + fread(ibuf + pir - pil, 1, SZ - pir + pil, stdin); pil = 0;
    if (pir < SZ) ibuf[pir++] = '\n';
}
...
void rd_integer(T &x) {
    ...
    bool minus = 0;
    if constexpr (std::is_signed<T>::value || std::is_same_v<T, __int128>) {
...
void rd(std::pair<T, U> &p) { return rd(p.first), rd(p.second); }
...
template <class... T> void rd(std::tuple<T...> &tpl) { rd_tuple(tpl); }
