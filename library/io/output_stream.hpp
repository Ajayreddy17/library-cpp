#ifndef AJAY_OUTPUT_STREAM
#define AJAY_OUTPUT_STREAM
...
namespace mitsuha::io {
    ...
    public:
        ...
        template <typename T> OutputStream& operator<<(const T& e) {
            if constexpr (mitsuha::is_writable_v<T>) os << e; else _print(e);
            return *this;
        }
        void print() { *this << '\n'; }
        template <typename Head, typename... Tail>
        void print(const Head& head, const Tail &...tails) { *this << head, ((*this << ' ' << tails), ...), *this << '\n'; }
        template <typename Iterable, std::enable_if_t<mitsuha::is_iterable_v<Iterable>, std::nullptr_t> = nullptr>
...
namespace mitsuha { using io::print, io::print_all; } // namespace mitsuha
#endif // AJAY_OUTPUT_STREAM